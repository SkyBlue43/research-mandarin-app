...
export function useCharacters(test: string | null, arrayIndex: number) {
    ...
    const changeWord = (num: string, simplified: string, traditional: string, pinyin: string, hint: string) => {
        setCurrentIndex(num);
    ...
    return {characters, currentIndex, currentSimplified, currentTraditional, currentPinyin, currentHint, changeWord}
}